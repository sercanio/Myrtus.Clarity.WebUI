...
// If you consider "uploadedAt desc" your default/no-custom-sorting scenario:
...
const MediaLibrary: React.FC<MediaLibraryProps> = ({ onSelect, onSelectCoverImage }) => {
  ...
  const [searchTerm] = useState(''); // Local filter string
  ...
  // Insert image in content (or wherever)
  ...
  // If user sorts in table "list" mode
  const handleTableChange = (sorter: { field?: string; order?: 'ascend' | 'descend' }) => {
    if (sorter.field && sorter.order) {
      ...
      // If the user resets sorting or if there's no field
      setDynamicRequest((prev) => ({
        ...prev,
        sort: [{ field: 'uploadedAt', dir: 'desc' }], // fallback default
      }));
    }
  };
                ...
                <Col xs={24} sm={12} md={8} lg={6} key={media.id}>
                  <Card
                    ...
                    actions={[
                      ...
                      >
                        {onSelect && (
                          <Button icon={<SelectOutlined />} onClick={() => handleSelect(media.blobUri)} type="link">
                            Select
                          </Button>
                        )}
                        {onSelectCoverImage && (
                          <Button icon={<PictureOutlined />} onClick={() => handleSelectCoverImage(media.blobUri)} type="link">
                            Set as Cover
                          </Button>
                        )}
                        <Button danger icon={<DeleteOutlined />} onClick={() => handleDelete(media)} type="link">
                          Delete
                        </Button>
                      </Space>,
                    ]}
          ...
          <Table
            ...
            onChange={(pagination, filters, sorter: any) => {
              if (sorter?.field && sorter.order) {
                ...
                // If user clears or there's no field => fallback
                setDynamicRequest((prev) => ({
                  ...prev,
